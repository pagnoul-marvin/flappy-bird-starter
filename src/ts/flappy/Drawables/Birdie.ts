...
export class Birdie extends Drawable implements IAnimatable {
    ...
    constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, sprite: HTMLImageElement, tubesPairs: TubesPair[], gameStatus:IGameStatus) {
    ...
    update(): void {
        this.frameCount++;
        if (this.frameCount >= settings.birdie.maxFrameInterval) {
            this.frameCount = 0;
    ...
    private checkCollisionWithTubes() {
        this.tubesPairs.forEach(tubesPair => {
            if (this.x + settings.birdie.width / 2 > tubesPair.x && this.x - settings.birdie.width / 2 < tubesPair.x + settings.tubes.sw) {
                if ((this.y - settings.birdie.height / 2) < tubesPair.topY + settings.tubes.sh || (this.y + settings.birdie.height / 2) > tubesPair.bottomY) {
